...
namespace Textadventure
{
    public static class File_Saving_And_Loading
    {
        ...
        {
            // Character_status noch einbauen sobald funktionsfähig

            List<string> saving_info_list = new List<string>();

            string places_available_str = string.Join(",", Place.places_available);
            string line_0 = $"Game State ->|{Program_Main.location_current}|{Program_Main.total_defeated_enemies}|{places_available_str}|";
            ...
            foreach (Character_Player player in Program_Main.party)
            {
                string line_1 = $"{player.character_name}|character data|{player.character_class}|{player.character_race}|{player.character_level}|{player.hit_dice}|{player.strength}|{player.dexterity}|" +
                                ...
                                $"{player.player_status.conditions["is_a_summon"]}|{player.player_status.conditions["fainted"]}|{player.player_status.conditions["alive"]}|";
            ...
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (line.StartsWith("Game State ->"))
                    {
                        ...
                        Program_Main.total_defeated_enemies = int.Parse(parts[2]);
                        Place.places_available = new List<string>(parts[3].Split(','));
                    }
                    else
                    {
                        switch (parts[1])
                        {
                            case "character data":
                                ...
                                current_player_loading.player_status.conditions["is_a_summon"] = bool.Parse(parts[16]);
                                current_player_loading.player_status.conditions["fainted"] = bool.Parse(parts[17]);
                                current_player_loading.player_status.conditions["alive"] = bool.Parse(parts[18]);
                                // erst testen dann die anderen speichern
