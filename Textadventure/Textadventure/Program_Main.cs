...
{   
    public class Program_Main
    {
        ...
        static string location_current = location_starting;
        ...
        static string[] items_weapons = { "Sword", "Dagger", "Bow", "Mace", "Staff" };
        static string[] items_armor = { "Shield", "Ring", "Necklace", "Cloak", "Plate", "Chainmail", "Studded Leather", "Leather", "Cloth" };
        static string[] items_potions = { "Potion of Healing", "Potion of Poison", "Potion of Strength", "Potion of Weakness" };
        static string[] items_scrolls = { "Scroll of Spell" }; // zugriff auf einen Zauber, mal sehen wie das genau wird // NICHT ARRAY 
        static string[] items_utility = { "Rope", "Wood", "Flint and Steel", "Bag", "Coin", "Map", "Small Key" };
        static string[] items_food = { "Fish", "Chicken", "SoupSoupSoup", "Beef", "Vegetables", "Herbs", "Water" };
        ...
        static List<string>? multi_word_strings;
        ...
        private static List<string> extract_multi_word_strings()
        {
            ...
            {
                multi_word_strings = new List<string>(); // Erstelle eine neue Liste
        ...
        private static string[] combine_multi_word_strings(string input)
        {
            ...
            foreach (var name in player_active_names)
            {
                ...
                {
                    input = input.Replace(name, name.Replace(" ", "_"), StringComparison.OrdinalIgnoreCase);
                }
            }
            if (multi_word_strings != null)
            {
                foreach (var multi_word in multi_word_strings)
                {
                    ...
                    {
                        input = input.Replace(multi_word, multi_word.Replace(" ", "_"), StringComparison.OrdinalIgnoreCase);
                    }
                }
            }
            return input.Split(' ').Select(part => part.Replace("_", " ")).ToArray();
        }
        ...
        public static string input_reader()
        {
            ...
            while (input_player == null || string.IsNullOrWhiteSpace(input_player))
            {
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine("ok let's give it a try.");
                        Console.WriteLine(); // save noch schreiben!
                        save_game();
                    }
                    else if (parts[0].ToLower() == "load")
                    {
                        Console.WriteLine("You want to load another game but I do not like to do that");
                    ...
                    else if (parts[0].ToLower() == "inventory")
                    {
                        found = false;
                        foreach (Character player in party)
                        {
                            if (player.character_name == parts[1])
                            {
                                Console.WriteLine();
                        ...
                        if (!found)
                        {
                            Console.WriteLine($"Player \"{parts[1]}\" not found in the party.");
                        }
                    }
                    else if (party.Any(p => p.character_name.Equals(parts[0], StringComparison.OrdinalIgnoreCase)) && parts[1] == "use")
                    {
                        foreach (Character player in party)
                        {
                    ...
                    else if (party.Any(p => p.character_name.Equals(parts[0], StringComparison.OrdinalIgnoreCase)) 
                    ...
                    else if (party.Any(p => p.character_name.Equals(parts[0], StringComparison.OrdinalIgnoreCase)) && parts[1] == "equip")
                    {
                        foreach (Character player in party)
                        {
                            if (player.character_name == parts[0])
                    ...
                    else if (party.Any(p => p.character_name.Equals(parts[0], StringComparison.OrdinalIgnoreCase)) && parts[1] == "unequip")
                    {
                        foreach (Character player in party)
                        {
                            if (player.character_name == parts[0])
        ...
        static void help_output()
        {
            ...
            Console.WriteLine("inventory [name] ) Displays the entire Inventory of a Player.");
            Console.WriteLine("use [item] ) Uses whatever you choose to use.");
            Console.WriteLine();
        ...
        public static void create_player()
        {
            ...
            for (int i = 0; i < picked_party_size; i++)
            {
                ...
                while (!party_name_complete)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(player_name) || IsRestrictedName(player_name) 
                            || party.Exists(p => p.character_name.ToLower() == player_name.ToLower()))
                    {
                        ...
                        else if (IsRestrictedName(player_name))
                        {
        ...
        static bool IsRestrictedName(string name)
        {
        ...
        static void start_menue()
        {

            /*
            Willkommenstext
            // switch 1 Sprache Wählen English / Deutsch
            // switch 2 Spiel Wählen Neu / Laden / Multiplayer
            type help falls du hilfe brauchst posten
            starten -> create Player
            falls Multiplayer warten auf andere Spieler
            Startsequenz
            */


            /*activate = rnd.check_probability(25);
        ...
        static void save_game()
        {
            ...
            Console.WriteLine("0) make a new Save File");
        ...
        public static void title_screen()
        {
            ...
            Console.WriteLine();
            Console.WriteLine();
            Console.WriteLine("Über 3500 Zeilen Code im Nebenbei!");
            Console.WriteLine();
            Console.WriteLine();
            Console.WriteLine("Tel:     0650 644 99 85");
            Console.WriteLine("E-Mail:  alexfleig777@gmail.com");
            Console.WriteLine();
            Console.WriteLine();
            Console.WriteLine();
            Console.WriteLine();
